...
namespace Space3x.InspectorAttributes.Editor.VisualElements
{
    ...
    public partial class InvokableField : TextField
    {
        ...
        private void GetMethodParameters()
        {
            ...
            var methodInfo = ReflectionUtility.FindFunction(Property.Name, Property, ref targetObject);
            if (methodInfo != null)
            {
        ...
        private bool ShowInputPrompt(Type parameterType, string parameterName)
        {
            return parameterType switch
            {
                ...
                #if UNITY_EDITOR
                not null when parameterType == typeof(Color) => TryShowPrompt<ColorField, Color>(parameterName),
                not null when parameterType == typeof(AnimationCurve) => TryShowPrompt<CurveField, AnimationCurve>(parameterName),
                not null when parameterType == typeof(Gradient) => TryShowPrompt<GradientField, Gradient>(parameterName),
        ...
        {
            InputValueDialog.Prompt<TField, TValue>(() => new TField() { label = parameterName }, OnParameterValueReceived);
            return true;
        }
        ...
        {
            InputValueDialog.Prompt<UnityEditor.UIElements.ObjectField, UnityEngine.Object>(() => new UnityEditor.UIElements.ObjectField()
            {
                label = parameterName,
