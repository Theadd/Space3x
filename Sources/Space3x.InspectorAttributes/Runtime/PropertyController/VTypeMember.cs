    ...
    public class VTypeMember : IPropertyNodeWithFlags
    {
        ...
        private bool Rebuild()
        {
...
#if UNITY_EDITOR
                if (typeof(DecoratorDrawerAdapter).IsAssignableFrom(drawer))
                    m_DecoratorDrawers.Add(drawer);
                else if (typeof(UnityEditor.DecoratorDrawer).IsAssignableFrom(drawer))
                    m_DecoratorDrawers.Add(drawer);
                else if (typeof(PropertyDrawerAdapter).IsAssignableFrom(drawer) || typeof(UnityEditor.PropertyDrawer).IsAssignableFrom(drawer))
#else
                if (typeof(DecoratorDrawerAdapter).IsAssignableFrom(drawer))
                    m_DecoratorDrawers.Add(drawer);
                else if (typeof(PropertyDrawerAdapter).IsAssignableFrom(drawer))
#endif
                {
                    m_DecoratorDrawers.Add(null);
            ...
            if (!isInvokableNode)
            {
                ...
                if (isArray && m_PropertyDrawerOnCollectionItems == null)
                    m_PropertyDrawerOnCollectionItems =
                        CachedDrawers.GetCustomDrawer(GetArrayOrListElementType(FieldType));
            }
            return true;
        }
